...
def encode_prompt(captions, text_encoders, tokenizers, is_train=True):
    prompt_embeds_list = []
    with torch.no_grad():
...
def main():
    ...
    controlnet.train().requires_grad_(True)
    ...
    dataset_obj = COCODataset('../data/COCO/train2017', annPath='../data/COCO/annotations_trainval2017/captions_train2017.json', res=1024)
    ...
    controlnet, optimizer, dataset_iterator, lr_scheduler = accelerator.prepare(
        controlnet, optimizer, dataset_iterator, lr_scheduler
    )
    ...
    with torch.no_grad():
        ...
        uc_prompt_embeds = uc["prompt_embeds"].to(dtype=weight_dtype)
        uc_pooled_prompt_embeds = uc["pooled_prompt_embeds"].to(dtype=weight_dtype)

    rg = RandomGenerator()
    
    if accelerator.is_main_process:
        with torch.no_grad():
            log_validation(pipeline, controlnet, args, accelerator, global_step)
    ...
    while True:
        ...
        if global_step % args.gradient_accumulation_steps == 0:
            ...
            cfg_condition = cfg_scale * torch.ones((args.train_batch_size, 3, 1024, 1024), device=accelerator.device)
        ...
        with accelerator.accumulate(controlnet):
            with torch.no_grad():
                ...
                # time ids
                add_time_ids = torch.cat([compute_time_ids(s, c) for s, c in zip([(1024, 1024)] * args.train_batch_size, [(0, 0)] * args.train_batch_size)])

                # Get the text embedding for conditioning
                c = compute_embeddings_fn(captions)
                c_prompt_embeds = c["prompt_embeds"].to(dtype=weight_dtype)
                c_pooled_prompt_embeds = c["pooled_prompt_embeds"].to(dtype=weight_dtype)

                # Predict the noise residual
                unet_added_conditions = {"time_ids": torch.cat([add_time_ids] * 2)}
            ...
            with torch.autocast('cuda'):
                down_block_res_samples, mid_block_res_sample = controlnet(
                    noisy_latents,
                    timesteps,
                    encoder_hidden_states=c_prompt_embeds,
                    added_cond_kwargs=unet_added_conditions, 
...
@torch.no_grad()
def log_validation(pipeline, controlnet, args, accelerator, global_step):
    ...
    with torch.autocast('cuda'):
        if global_step == 0:
            ...
            images = torch.from_numpy(np.stack(images)).permute(0, 3, 1, 2).float() / 255
            images = make_grid(images, nrow=4, padding=0)
            save_image(images, os.path.join(args.project_dir, 'images', f"grid_pnp_{global_step}.png"))
