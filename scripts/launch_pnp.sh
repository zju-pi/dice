# PnP Training on SD15
MODEL_NAME="Lykon/DreamShaper"
n_gpus=8
acc_steps=1
num_updates=8000
accelerate launch --num_processes=$n_gpus --mixed_precision="bf16" train_sd15_pnp.py \
...
# PnP Training on SDXL
MODEL_NAME="stabilityai/stable-diffusion-xl-base-1.0"
n_gpus=8
acc_steps=1
num_updates=4000
accelerate launch --num_processes=$n_gpus --mixed_precision="bf16" train_sdxl_pnp.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --variant="fp16" \
  --enable_xformers_memory_efficient_attention \
  --gradient_accumulation_steps=$acc_steps \
