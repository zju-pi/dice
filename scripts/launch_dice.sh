# DICE Training on SD15
MODEL_NAME="Lykon/DreamShaper"
n_gpus=8
acc_steps=1
num_updates=8000
accelerate launch --num_processes=$n_gpus --mixed_precision="bf16" train_sd15_dice.py \
...
# DICE Training on SDXL
MODEL_NAME="stabilityai/stable-diffusion-xl-base-1.0"
n_gpus=8
acc_steps=1
num_updates=4000
accelerate launch --num_processes=$n_gpus --mixed_precision="bf16" train_sdxl_dice.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --variant="fp16" \
  --enable_xformers_memory_efficient_attention \
  --gradient_accumulation_steps=$acc_steps \
...
desc="${out_folder}"
ref="/Path/to/reference/statistis"
accelerate launch --num_processes=$n_gpus sample.py --model_name=$model_name --model_path=$model_path --model_id=$model_id --method=$method --steps=$steps --alpha=$alpha --batch_gpu=$batch_gpu;
accelerate launch --num_processes=$n_gpus ./evaluation/eval_fid.py calc --images=$image_path --ref=$ref --desc=$desc;
accelerate launch --num_processes=$n_gpus ./evaluation/eval_clip_score.py calc --images=$image_path --desc=$desc;
accelerate launch --num_processes=$n_gpus ./evaluation/eval_aes_score.py calc --images=$image_path --desc=$desc;
