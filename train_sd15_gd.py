        ...
        with accelerator.accumulate(unet):
            with torch.no_grad():
                ...
                with torch.autocast('cuda'):
                    target = unet_tea(torch.cat([noisy_latents] * 2), torch.cat([timesteps] * 2), torch.cat([uc_neg if args.use_neg else uc, c]), return_dict=False)[0]
                    target_uncond, target_text = target.chunk(2)
                    target = target_uncond + cfg_scale * (target_text - target_uncond)

            with torch.autocast('cuda'):
                # Compute loss
                model_pred = unet(noisy_latents, timesteps, c, return_dict=False, cfg_condition=cfg_condition)[0]
