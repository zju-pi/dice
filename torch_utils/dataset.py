...
class COCODataset(Dataset):
    def __init__(self,
        path,                   # Path to directory or zip.
        annPath,                # Path to json.
        resolution      = None, # Ensure specific resolution, None = highest available.
        use_pyspng      = True, # Use pyspng if available?
        res             = 512,  # Resolution of the image
        **super_kwargs,         # Additional arguments for the Dataset base class.
    ):
        from torch_utils.coco import COCO
    ...
    def _load_raw_image(self, raw_idx):
        ...
        image = (255 * transform(image)).numpy().astype(np.uint8)

        # image = image.transpose(2, 0, 1) # HWC => CHW

        img_id = int(os.path.splitext(fname)[0].split('/')[-1])
        imgIds = self.coco.getImgIds(imgIds=[img_id])
        img = self.coco.loadImgs(imgIds[np.random.randint(0,len(imgIds))])[0]
        annIds = self.coco.getAnnIds(imgIds=img['id'])
        label = self.coco.loadAnns(annIds[np.random.randint(0,len(annIds))])[0]['caption']
